(() => {
  ...
  function addCards(){
    ...
    if(!document.getElementById("vt-home-card")){const card=document.createElement("button");card.id="vt-home-card";card.className="vt-card";card.innerHTML=`<div style="font:1.25rem 'Cormorant Garamond',serif;color:var(--burgundy)">${t("cardTitle")}</div><div style="color:var(--muted);font-size:.82rem">${t("cardDesc")}</div>`;card.onclick=openGuest;grid.insertBefore(card,admin)}
    if(!document.getElementById("vt-gallery-card")){const card=document.createElement("button");card.id="vt-gallery-card";card.className="vt-card";card.innerHTML=`<div style="font:1.25rem 'Cormorant Garamond',serif;color:var(--burgundy)">${t("galleryTitle")}</div><div style="color:var(--muted);font-size:.82rem">${t("galleryDesc")}</div>`;card.onclick=openGallery;grid.insertBefore(card,admin)}
  }
