(() => {
  ...
  const css = `
    ...
    .vt-shell{width:min(100%,780px);height:100dvh;max-height:100dvh;margin:0 auto;box-sizing:border-box;overflow-x:hidden;overflow-y:auto!important;-webkit-overflow-scrolling:touch;overscroll-behavior-y:contain;touch-action:pan-y;padding:calc(var(--event-header-reserve,96px) + 10px) 10px max(120px,calc(70px + env(safe-area-inset-bottom)))}.vt-toolbar{display:grid;gap:8px;margin-bottom:14px}.vt-overlay-actions{display:flex;align-items:center;justify-content:space-between;gap:10px}.vt-panel{background:var(--white);border-radius:22px;padding:22px;box-shadow:0 6px 30px rgba(92,42,30,.16)}
    ...
    @media (max-width:650px){
      ...
      .vt-shell{width:100%;height:100dvh;max-height:100dvh;padding:calc(var(--event-header-reserve,104px) + 10px) 8px max(150px,calc(86px + env(safe-area-inset-bottom)));scroll-padding-top:var(--event-header-reserve,104px)}
