(() => {
  ...
  const css = `
    ...
    .vt-overlay{position:fixed;inset:0;z-index:2147483645;background:linear-gradient(160deg,var(--cream),var(--blush));height:100dvh;max-height:100dvh;box-sizing:border-box;overflow:hidden!important;padding:0;font-family:Jost,Arial,sans-serif;color:var(--text)}
    .vt-shell{width:min(100%,780px);height:100dvh;max-height:100dvh;margin:0 auto;box-sizing:border-box;overflow-x:hidden;overflow-y:auto!important;-webkit-overflow-scrolling:touch;overscroll-behavior-y:contain;touch-action:pan-y;padding:max(12px,env(safe-area-inset-top)) 10px max(120px,calc(70px + env(safe-area-inset-bottom)))}.vt-toolbar{display:grid;gap:8px;margin-bottom:14px}.vt-overlay-actions{display:flex;align-items:center;justify-content:space-between;gap:10px}.vt-language-slot{width:100%;display:flex;justify-content:flex-end}.vt-language-slot:empty{display:none}.vt-overlay #wedding-language-switcher{position:static!important;inset:auto!important;top:auto!important;right:auto!important;left:auto!important;width:auto!important;max-width:max-content!important;display:inline-flex!important;justify-content:flex-start!important;gap:4px!important;margin:0!important;padding:5px!important;box-sizing:border-box!important;transform:none!important;border-radius:999px!important}.vt-overlay #wedding-language-switcher button{flex:0 0 auto!important;min-width:0!important;padding:8px 11px!important;font-size:12px!important}.vt-panel{background:var(--white);border-radius:22px;padding:22px;box-shadow:0 6px 30px rgba(92,42,30,.16)}
    ...
    @media (max-width:650px){
      ...
      .vt-shell{width:100%;height:100dvh;max-height:100dvh;padding:calc(env(safe-area-inset-top) + 8px) 8px max(150px,calc(86px + env(safe-area-inset-bottom)));scroll-padding-top:8px}
      ...
      .vt-overlay #wedding-language-switcher{border-radius:999px!important;padding:4px!important}
      .vt-overlay #wedding-language-switcher button{flex:0 0 auto!important;padding:7px 10px!important;font-size:12px!important}
      ...
      .vt-gallery-item video{pointer-events:none!important;display:block;touch-action:pan-y!important}
      .vt-actions{gap:7px}
      .vt-btn{padding:10px 14px}
      .vt-heart-actions{gap:8px}
      .vt-heart-label{inset:24% 8% 18%;font-size:.78rem}
      .vt-heart-icon{font-size:1.5rem}
    }
    @media (max-width:390px){
      .vt-overlay{padding-left:8px;padding-right:8px}
      .vt-panel{padding:14px 9px}
    }
    ...
    .vt-gallery-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(230px,1fr));gap:16px;align-items:start}.vt-gallery-item{background:#fff;border:1px solid #f1d8cf;border-radius:16px;padding:10px;box-shadow:0 2px 10px rgba(92,42,30,.08);touch-action:pan-y;scroll-margin-top:110px}.vt-gallery-item video{width:100%;aspect-ratio:16/9;background:#000;border-radius:11px;object-fit:contain;touch-action:pan-y}
