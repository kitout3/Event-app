...
const weddingUrl = (slug, admin = false) => `${APP_BASE}?w=${encodeURIComponent(slug)}${admin ? '#admin' : ''}`;
...
export default function SoftwareAdmin(){
  ...
  if(!authorized)return <Shell><div style={{maxWidth:390,width:"100%",background:"#fff",padding:30,borderRadius:20,boxShadow:"0 15px 50px #0002"}}><div style={{fontSize:12,letterSpacing:2,textTransform:"uppercase",opacity:.55}}>Administration logiciel</div><h1 style={{fontFamily:"Georgia,serif",fontWeight:400,fontSize:34,margin:"7px 0 20px"}}>Gestion des mariages</h1><a href={APP_BASE} style={{display:"inline-block",color:"#632c3b",fontSize:14,marginBottom:20}}>← Retour à l’accueil</a><div style={{display:"grid",gap:10}}><input style={inputStyle} type="email" aria-label="Adresse email" autoComplete="username" placeholder="Email" value={email} onChange={e=>setEmail(e.target.value)}/><input style={inputStyle} type="password" aria-label="Mot de passe" autoComplete="current-password" placeholder="Mot de passe" value={password} onChange={e=>setPassword(e.target.value)} onKeyDown={e=>e.key==="Enter"&&login()}/>{error&&<p style={{color:"#a33",fontSize:13}}>{error}</p>}<button style={btn(true)} onClick={login}>Se connecter</button></div></div></Shell>;
  ...
  return <div style={{minHeight:"100vh",background:"#f7f3f0",color:"#38231c",fontFamily:"Arial,sans-serif"}}>
    <header style={{background:"#24140e",color:"#fff",padding:"18px clamp(18px,4vw,48px)",display:"flex",gap:12,alignItems:"center",flexWrap:"wrap"}}>
      <div style={{flex:1}}><div style={{fontSize:11,letterSpacing:2,textTransform:"uppercase",opacity:.55}}>Espace Mariage · Administration</div><h1 style={{fontFamily:"Georgia,serif",fontWeight:400,margin:"4px 0 0"}}>Tous les mariages</h1></div>
    ...
    <main style={{maxWidth:1180,margin:"0 auto",padding:"28px 16px"}}>
      ...
      <section style={{background:"#fff",padding:22,borderRadius:18,border:"1px solid #eaded7"}}>
        ...
        {!loading&&filtered.length===0&&<p role="status">{search?"Aucun mariage ne correspond à cette recherche.":"Aucun mariage pour le moment."}</p>}<div style={{display:"grid",gap:10}}>{filtered.map(w=><div key={w.id} style={{border:"1px solid #eaded7",borderRadius:14,padding:15,display:"flex",gap:14,alignItems:"center",flexWrap:"wrap"}}><div style={{flex:"1 1 300px"}}><strong style={{fontFamily:"Georgia,serif",fontSize:20}}>{w.name}</strong><div style={{fontSize:13,opacity:.65,marginTop:3}}>{w.date||"Date non renseignée"} · {w.slug}</div><div style={{fontSize:12,opacity:.55,marginTop:3}}>Admin : {w.adminEmail||"—"} · <strong>{w.active?"Actif":"Désactivé"}</strong></div></div><div style={{fontSize:13}}>{w.photoCount||0} photos · {w.videoCount||0} vidéos</div><button style={btn()} onClick={()=>startEdit(w)}>Modifier</button><button style={btn()} onClick={()=>toggleActive(w)}>{w.active?"Désactiver":"Activer"}</button><button style={btn()} onClick={()=>resetAccess(w)}>Réinitialiser accès</button><button style={btn()} onClick={()=>window.open(weddingUrl(w.slug),"_blank","noopener,noreferrer")}>Application</button><button style={btn(true)} onClick={()=>window.open(weddingUrl(w.slug,true),"_blank","noopener,noreferrer")}>Admin mariage</button><button style={{...btn(),background:"#fff0ed",color:"#a33"}} disabled={deleting===w.id} onClick={()=>removeWedding(w)}>{deleting===w.id?"Suppression…":"Supprimer"}</button></div>)}</div>
      </section>
    </main>
  </div>;
}
